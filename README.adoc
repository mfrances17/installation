...
The following will provide information on how to install Integreatly on a openshift cluster via a master host.


#### 1. Sign into your master host and become root of the host

[source,shell]

```
ssh -i /path/ocpkey.pem ec2-user@master.NAME-GUID.openshiftworkshop.com

sudo -i
```


#### 2. Clone installation GIT repository on the master host
...
Depending on the environment you're installing into, there are different pre-existing inventory files that can be used for installation.

Usually, when manually running an installation, the `inventories/pds.template` file is the inventory that should be used. The default contents of this file can normally be used without modification when running from the master host.

Create a copy of he `inventories/pds.template` template, in case it needs to be modified:

```
cp inventories/pds.template inventories/pds
```
...
[source,shell]
----
 ansible-playbook -i inventories/pds playbooks/install.yml -e github_client_id=<your_client-id> -e github_client_secret=<your_client_secret>
----
...
[source,shell]
----
$ ansible-playbook -i inventories/pds playbooks/install.yml
----
====
...
[source,shell]
----
$ ansible-playbook -i inventories/pds playbooks/uninstall.yml
----
